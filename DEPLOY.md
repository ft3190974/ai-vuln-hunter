...
**Q: 上传大文件后服务崩溃/无响应？**
- demo 环境上传限制 **20MB**（大包解压 + 遍历 + LLM 分析易内存溢出）
- 前端选择文件时会预检大小，超限直接拦截并提示
- 如需扫描更大包：在服务器内存充足的前提下，修改 `web/server/routes/upload.js` 的 `MAX_UPLOAD_BYTES`

**Q: 扫描中切换到其他页面，任务会丢吗？**
- 不会。扫描状态用 sessionStorage 持久化，切到「态势总览」等页面再回来，上传的文件、进度时间线、任务状态都还在，轮询自动继续。
- 关闭浏览器标签页后自动清空（下次访问是干净初始态）。

**Q: 扫描卡住了怎么办？**
- 扫描页有「✖ 终止任务」按钮，点击即可取消（前端停止轮询 + 后端标记失败 + 删除任务）
- 超时（60 秒无响应）的任务会自动标记为失败，不会永远卡在「运行中」
- 注意：LLM 调用一旦发出无法真正中断，终止后后台可能还会跑一会儿，但结果会被丢弃
