...
### 1. 扫描任务页
- **上传文件**（推荐）：拖拽 `.zip`（源码包）/ `.jar` / `.bin` → 自动解压 → 开始挖掘（限制 20MB）
- **指定路径**：填本地源码目录
- **粘贴代码**：直接粘贴代码片段
- **Web 渗透**：填 URL + 可选登录认证（用户名/密码/Cookie）
- 扫描中可随时**切换导航页**，任务状态不丢失（sessionStorage 持久化）
- 扫描中可点 **「✖ 终止任务」** 取消当前任务
- 超时任务自动标记为失败，不会永远卡在「运行中」

### 2. 任务管理页
- 查看所有扫描任务，按任务查看漏洞（漏洞按任务隔离）
- 删除任务时**联动清理**该任务的漏洞 + 同步态势总览
- **「🧹 清理残留数据」**：一键清掉任务已删但漏洞残留的历史数据
